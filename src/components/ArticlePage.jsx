...
function ArticlePage(props) {
  ...
  const fetchArticle = async (slug) => {
    setIsLoading(true);
    const resArticleBySlug = await getArticleBySlug(
      slug,
  ...
  const handleFavorite = async () => {
    ...
    if (article.favorited) {
      ...
      resArticleUpdate = await favoriteArticle(
        article?.slug,
        userLogin?.user?.token
      );
    }
  ...
  const handlePostComment = async () => {
    ...
    setIsLoading(true);
    const resComment = await postCommentArticle(
      dataComment,
      article?.slug,
      userLogin?.user?.token
    );
  ...
  const handleDeleteComment = async (idComment) => {
    setIsLoading(true);
    const resDeleteCmt = await deleteComment(
      article?.slug,
      idComment,
      userLogin?.user?.token
    );
    let cloneListComment = listComment;
    cloneListComment = cloneListComment.filter(
  ...
  const handleDeleteArticle = async () => {
    setIsLoading(true);
    const resDeleteArticle = await deleteArticle(
      article?.slug,
        ...
        <div className="container">
          ...
          <div className="article-meta">
            ...
            <div className="info">
              ...
              <span className="date">
                {isLoading ? formattedDate : "December 9, 2022"}
              </span>
            </div>
      ...
      <div className="container page">
        <div className="row article-content">
          <div className="col-md-12">
            <p>
              <span style={{ whiteSpace: "pre-line" }}>{article?.body}</span>
            </p>
            <div className="article-tagList">
              {article?.tagList?.map((tag, index) => (
                <span key={index + "-" + tag} className="tagList">
                  {tag}
                </span>
              ))}
            </div>
        ...
        {userLogin?.user?.token ? (
          ...
          <div>
            <NavLink to="/login">Sign in</NavLink> or{" "}
            <NavLink to="/register">sign up</NavLink> to add comments on this
            article.
          </div>
        )}
      </div>
