...
import { getListArticleByOffset, getPopularTag, getTag, getYouFeed } from '../services/apiServices';
...
function HomePage(props) {
    ...
    const fetchListArticle = async (offsetTarget) => {
        setIsLoading(true)
        const resListArticle = getListArticleByOffset(offsetTarget || 0, userLogin?.user?.token || JSON.parse(infoUser)?.user?.token)
        const popularTag = getPopularTag(userLogin?.user?.token || JSON.parse(infoUser)?.user?.token)
            ...
            <div className="container page home-container">
                <div className="row">
                    <div className="col-md-9">
                            ...
                            <ul className="nav nav-pills outline-active">
                                {
                                    ...
                                    <li className="nav-item" onClick={() => {
                                        ...
                                        <a className={`nav-link  ${tagActive === "yourfeed" ? "active" : ""}`} >Your Feed</a>
                                    </li>
                                }
                                <li className="nav-item" onClick={() => {
                                    if (tagActive !== "global") {
                                    ...
                                    <a className={`nav-link  ${tagActive === "global" ? "active" : ""}`} >Global Feed</a>
                                </li>

                                {
                                    tagActive !== "global" && tagActive !== "yourfeed" &&
                                    <li className="nav-item">
                                        <a className={`nav-link ${tagActive ? "active" : ""}`} ># {tagActive}</a>
                                    </li>
                                }
                            </ul>
                        </div>
                        ...
                        {
                            tagActive === "global" && listArticle && listArticle.length > 0 &&
                    ...
                    <div className="col-md-3">
                        <div className="sidebar">
                            ...
                            <div className="tag-list">
                                {
                                    popularTag && popularTag.map((tag, index) => {
                                        return (
                                            <NavLink key={index + "-" + tag} onClick={() => {
                                                if (tagActive !== tag) {
                                                    handleSetPopularTag(tag)
                                                }
                                            }} className={`tag-pill tag-default `}>{tag}</NavLink>
                                        )
                                    })
                                }
                            </div>
