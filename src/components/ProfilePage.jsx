...
import { deleteUnfollowUser, getArticleByUser, getFavoritedUser, postFollowUser } from '../services/apiServices';
...
function ProfilePage(props) {
    ...
    const fetchArticleByUsername = async (offset) => {
        ...
        setIsLoading(true)
        const resArticle = await getArticleByUser(userPrf, offset || 0, JSON.parse(infoUser)?.user?.token)
    ...
    const fetchFavoriteArticle = async (offset) => {
        ...
        // }
        setIsLoading(true)
        const resFavorited = await getFavoritedUser(params?.username, offset || 0, JSON.parse(infoUser)?.user?.token)
    ...
    const handleFollow = async () => {
        ...
        if (profileUser?.following) {
            resFollowUpdate = await deleteUnfollowUser(profileUser?.username, userLogin?.user?.token)
        } else {
            resFollowUpdate = await postFollowUser(profileUser?.username, userLogin?.user?.token)
        }
                ...
                <div className="container">
                    <div className="row">
                        <div className="col-xs-12 col-md-10 offset-md-1">
                            <img src={params?.username === JSON.parse(infoUser)?.user?.username ? JSON.parse(infoUser)?.user?.image : profileUser?.image} className="user-img" />
                            <h4>{params?.username === JSON.parse(infoUser)?.user?.username ? JSON.parse(infoUser)?.user?.username : profileUser?.username}</h4>
                            <p>
                                <span style={{ whiteSpace: 'pre-line' }}>
                                    {params?.username === JSON.parse(infoUser)?.user?.username ? userLogin?.user?.bio : profileUser?.bio}
                                </span>
                            </p>
                            {params?.username === JSON.parse(infoUser)?.user?.username ?
                                ...
                                <button className={`btn btn-sm action-btn ${profileUser?.following ? "btn-secondary" : "btn-outline-secondary"} `}
            ...
            <div className="container">
                <div className="row">
                    <div className="col-xs-12 col-md-10 offset-md-1">
                        <div className="articles-toggle">
                            <ul className="nav nav-pills outline-active">
                                <li className="nav-item">
                                    <a className={`nav-link ${typeArticle === "my" ? "active" : ""}`}
                                        onClick={() => handleChangeTypeArticle("my")}
                                    >My Articles</a>
                                </li>
                                <li className="nav-item">
                                    <a className={`nav-link ${typeArticle === "favorite" ? "active" : ""}`}
                        ...
                        {
                            listArticle && listArticle.map((article, index) => {
