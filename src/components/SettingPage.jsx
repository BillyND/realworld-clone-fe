...
function SettingPage(props) {
  ...
  const [imageLink, setImageLink] = useState(
    userLogin?.user?.image || JSON.parse(infoUser)?.user?.image
  );
  const [username, setUsername] = useState(
    userLogin?.user?.username ||
      userLogin?.user?.email ||
      JSON.parse(infoUser)?.user?.username ||
      JSON.parse(infoUser)?.user?.email ||
      null
  );
  const [email, setEmail] = useState(
    userLogin?.user?.email || JSON.parse(infoUser)?.user?.email
  );
  const [password, setPassword] = useState(
    userLogin?.user?.password || JSON.parse(infoUser)?.user?.password
  );
  const [bio, setBio] = useState(
    userLogin?.user?.bio || JSON.parse(infoUser)?.user?.bio || ""
  );
  ...
  const handleUpdateInfo = async () => {
    ...
    const resUpdateInfo = await updateInfoUser(
      dataUpdate,
