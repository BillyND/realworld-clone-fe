...
function EditArticlePage(props) {
  ...
  const fetchEditArticle = async (slug) => {
    setIsLoading(true);
    try {
      const resEditArticle = await getArticleBySlug(
        slug,
  ...
  const handlePostArticle = async () => {
    ...
    try {
      ...
      if (param.slug) {
        const resEdit = await editArticle(
          param.slug,
          dataPostArticle,
          JSON.parse(infoUser)?.user?.token
        );
        ...
        const resPostNew = await postNewArticle(
          dataPostArticle,
          JSON.parse(infoUser)?.user?.token
        );
        console.log(">>res:", resPostNew);
        // if (resPostNew?.status === 200) {
        navigate(`/article/${resPostNew.data.article.slug}`);
        // } else {
        //   toast.error(resPostNew);
        //   console.log(">>error", resPostNew);
        // }
      }
    } catch (error) {
      console.error(">>>error:", error);
            ...
            <form>
              <fieldset>
                ...
                <div className="tag-list-new-article">
                  {tagListPost &&
                    tagListPost.length > 0 &&
                    tagListPost.map((tag, index) => {
                      return (
                ...
                <button
                  className="btn btn-lg pull-xs-right btn-primary btn-push-article"
                  type="button"
