...
import { deleteFavoriteArticle, favoriteArticle } from '../services/apiServices';
...
function ArticleCard(props) {
    ...
    const handleFavorite = async (slug, isFavorited) => {
        ...
        if (isFavorited) {
            resArticleUpdate = await deleteFavoriteArticle(slug, userLogin?.user?.token)
        } else {
            resArticleUpdate = await favoriteArticle(slug, userLogin?.user?.token)
        }
        const idxArticleUpdate = listArticle.findIndex(article => article.slug === resArticleUpdate.data.article.slug)
        listArticle[idxArticleUpdate].favorited = resArticleUpdate.data.article.favorited
        listArticle[idxArticleUpdate].favoritesCount = resArticleUpdate.data.article.favoritesCount
    ...
    return (
        <div className="article-preview">
            <div className="article-meta">
                <NavLink to={`/profile/${article?.author?.username}`} onClick={() => setIsLoading(true)}><img src={article?.author?.image} /></NavLink>
                    ...
                    <div className='article-tagList'>
                        {
                            article?.tagList?.map((tag, index) => (
                                <div key={index + "-" + tag} className='tagList'>{tag}</div>
                            ))
                        }
                    </div>
                </div>
