...
const getListArticleByOffset = (offset, token) =>
    api.get(`/articles?limit=10&offset=${offset}`, token && token && tokenHeaders(token));

const getArticleBySlug = (slug, token) =>
    api.get(`/articles/${slug}`, token && token && tokenHeaders(token));

const postLogin = (infoUser) =>
    api.post(`/users/login`, infoUser);

const postRegister = (dataRegister) =>
    api.post(`/users`, dataRegister);

const getArticleByUser = (username, offset, token) =>
    api.get(`/articles?author=${username}&limit=5&offset=${offset}`, token && token && tokenHeaders(token));

const getPopularTag = (token) =>
    api.get("/tags", token && token && tokenHeaders(token));

const favoriteArticle = (slug, token) =>
    api.post(`/articles/${slug}/favorite`, null, token && token && tokenHeaders(token));

const deleteFavoriteArticle = (slug, token) =>
    api.delete(`/articles/${slug}/favorite`, token && token && tokenHeaders(token));
...
const postFollowUser = (username, token) =>
    api.post(`/profiles/${username}/follow`, null, token && token && tokenHeaders(token));

const deleteUnfollowUser = (username, token) =>
    api.delete(`/profiles/${username}/follow`, token && token && tokenHeaders(token));

const postCommentArticle = (comment, slug, token) =>
    api.post(`/articles/${slug}/comments`, comment, token && token && tokenHeaders(token));

const deleteComment = (slug, idComment, token) =>
    api.delete(`/articles/${slug}/comments/${idComment}`, token && token && tokenHeaders(token));
...
const getFavoritedUser = (username, offset, token) =>
    api.get(`/articles?favorited=${username}&limit=5&offset=${offset}`, token && token && tokenHeaders(token));
